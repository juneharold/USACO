...
void solve() {
    int n; cin >> n;
    vector<int> a(n+1, 0);
    int cnt_pos=0, cnt_neg=0, mx_pos=0, mn_neg=0;
    for (int i=1; i<=n; i++) {
        cin >> a[i];
        if (a[i]<0) cnt_neg++;
        else if (a[i]>0) cnt_pos++;
    ...
    vector<pii> ans;
    if (cnt_neg<=7 && cnt_pos>0) {
        int idx=0;
        for (int i=1; i<=n; i++) {
            if (a[i]>a[idx]) idx=i;
        }
        while (a[idx]<abs(mn_neg)) {
        ...
        for (int i=1; i<=n; i++) {
            if (a[idx]<0 && a[i]>0) ans.push_back({i, idx});
            else if (a[idx]>0 && a[i]<0) ans.push_back({i, idx});
        }
        if (a[idx]>0) for (int i=2; i<=n; i++) ans.push_back({i, i-1});
        else for (int i=n-1; i>=1; i--) ans.push_back({i, i+1});
    }
    cout << ans.size() << "\n";
