...
void solve() {
    ...
    int cnt_diff=0;
    for (int i=0; i<n/2; i++) {
        if (s[i]!=s[n-1-i]) cnt_diff++;
    }
    string ans="";
    for (int i=0; i<cnt_diff; i++) ans+="0";
    if (n%2==1) {
        for (int i=cnt_diff; i<n/2; i++) ans+="1";
    }
    else {
        for (int i=cnt_diff; i<n/2; i++) {
            if ((i-cnt_diff)%2==0) ans+="1";
            else ans+="0";
        }
    }
    ...
    if (n%2==1) {
        ...
        cout << "11";
        for (int i=n/2-1; i>=0; i--) cout << ans[i];
    }
    else {
        for (int i=0; i<n/2; i++) cout << ans[i];
        if ((n/2-cnt_diff)%2==0) cout << "1";
        else cout << "0";
        for (int i=n/2-1; i>=0; i--) cout << ans[i];
    }
    cout << "\n";
