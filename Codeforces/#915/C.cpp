...
void solve() {
    ...
    for (int i=0; i<n; i++) letters[s[i]-'a'].push_back(i);
    vector<pair<int, char>> largest;
    for (int i=25; i>=0; i--) {
        for (int pos: letters[i]) {
            if (largest.empty() || pos>largest.back().fs) largest.push_back({pos, s[pos]});
        }
    }
    string final_string=s;
    int sub=0;
    for (int i=0; i<largest.size(); i++) {
        //cout << largest[i].fs << ' ' << largest[i].sc << endl;
        int pos=largest[largest.size()-1-i].fs, letter=largest[i].sc;
        final_string[pos]=letter;
        if (letter==largest[0].sc) sub++;
    }
    //cout << final_string << endl;
    bool sorted=true;
    for (int i=1; i<n; i++) {
        if (final_string[i]<final_string[i-1]) sorted=false;
    }
    if (sorted) cout << largest.size()-sub << "\n";
    else cout << -1 << "\n";
}
